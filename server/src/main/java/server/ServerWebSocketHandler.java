        ...
        try {
            ...
            if (command.getCommandType() == UserGameCommand.CommandType.CONNECT) {
                ...
                if (sessions != null) {
                    ...
                    System.out.println(username + " left game ");
                }

                GameData gameData = new GameMySqlDataAccess().joinGame(gameID);
                GameMySqlDataAccess dao = new GameMySqlDataAccess();

                try {
                    if (username.equals(gameData.whiteUsername())) {
                        dao.clearPlayerFromGame(gameID, "WHITE");
                    } else if (username.equals(gameData.blackUsername())) {
                        dao.clearPlayerFromGame(gameID, "BLACK");
                    }
                } catch (DataAccessException e) {
                    sendError(session, "Failed to update player status in database.");
                }
            ...
            System.err.println("Fatal WebSocket command error: " + e.getMessage());
            try {
                sendError(session, "Fatal server error: " + e.getMessage());
